...
fun rowDivision(row: String): Int? {
    val numbers = row.split("\t").map { it.toInt() }
    if (numbers.isEmpty()) return null

    for (x in numbers) {
        numbers.minus(x).filter { x % it == 0 }.forEach { return x/ it }
    }
    return null
}

fun checkSpreadsheet(spreadSheet: String): Int {
    return spreadSheet.split("\n").map { rowDivision(it) ?: 0}.sum()
}